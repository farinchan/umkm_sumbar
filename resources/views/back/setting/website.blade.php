                ...
                <div class="card-body">
                    ...
                    <div class="tab-content" id="myTabContent">
                                    ...
                                    <div class="col-md-9">
                                        <div class="image-input image-input-outline" data-kt-image-input="true"
                                            ...
                                            <div class="image-input-wrapper w-125px h-125px "
                                                style="background-image: url('{{ Storage::url('images/setting/' . $setting->logo) }}')">
                                            </div>
                                            <label
                                ...
                                <div class="row fv-row mb-7">
                                    ...
                                    <div class="col-md-9">
                                        <div class="image-input image-input-outline" data-kt-image-input="true"
                                            style="background-image: url('{{ asset('back/media/svg/avatars/blank.svg') }}')">
                                            <div class="image-input-wrapper w-125px h-125px "
                                                style="background-image: url('{{ Storage::url('images/setting/' . $setting->favicon) }}')">
                                            </div>
                                            <label
                                                class="btn btn-icon btn-circle btn-active-color-primary w-25px h-25px bg-body shadow"
                                                data-kt-image-input-action="change" data-bs-toggle="tooltip"
                        ...
                        <div class="tab-pane fade " id="kt_ecommerce_settings_general" role="tabpanel">
                            <form id="kt_ecommerce_settings_general_form" class="form" action="{{ route("admin.setting.information-update") }}" method="POST">
                                @method("PUT")
                                @csrf
                                <div class="row mb-7">
                                    <div class="col-md-9 offset-md-3">
...
@section('scripts')
    <script>
        ...
        const quill_privacy_policy = new Quill('#editor_privacy_policy', {
            theme: 'snow'
        });
        var quillEditor_privacy_policy = document.getElementById('privacy_policy');
        quillEditor_privacy_policy.value = quill_privacy_policy.root.innerHTML;
        quill_privacy_policy.on('text-change', function() {
            quillEditor_privacy_policy.value = quill_privacy_policy.root.innerHTML;
        });

        const quill_terms_and_conditions = new Quill('#editor_terms_and_conditions', {
            theme: 'snow'
        });
        var quillEditor_terms_and_conditions = document.getElementById('terms_and_conditions');
        quillEditor_terms_and_conditions.value = quill_terms_and_conditions.root.innerHTML;
        quill_terms_and_conditions.on('text-change', function() {
            quillEditor_terms_and_conditions.value = quill_terms_and_conditions.root.innerHTML;
        });

        const quill_return_policy = new Quill('#editor_return_policy', {
            theme: 'snow'
        });
        var quillEditor_return_policy = document.getElementById('return_policy');
        quillEditor_return_policy.value = quill_return_policy.root.innerHTML;
        quill_return_policy.on('text-change', function() {
            quillEditor_return_policy.value = quill_return_policy.root.innerHTML;
        });

        const quill_shipping_policy = new Quill('#editor_shipping_policy', {
            theme: 'snow'
        });
        var quillEditor_shipping_policy = document.getElementById('shipping_policy');
        quillEditor_shipping_policy.value = quill_shipping_policy.root.innerHTML;
        quill_shipping_policy.on('text-change', function() {
            quillEditor_shipping_policy.value = quill_shipping_policy.root.innerHTML;
        });

        const quill_cancellation_policy = new Quill('#editor_cancellation_policy', {
            theme: 'snow'
        });
        var quillEditor_cancellation_policy = document.getElementById('cancellation_policy');
        quillEditor_cancellation_policy.value = quill_cancellation_policy.root.innerHTML;
        quill_cancellation_policy.on('text-change', function() {
            quillEditor_cancellation_policy.value = quill_cancellation_policy.root.innerHTML;
        });

        const quill_refund_policy = new Quill('#editor_refund_policy', {
            theme: 'snow'
        });
        var quillEditor_refund_policy = document.getElementById('refund_policy');
        quillEditor_refund_policy.value = quill_refund_policy.root.innerHTML;
        quill_refund_policy.on('text-change', function() {
            quillEditor_refund_policy.value = quill_refund_policy.root.innerHTML;
        });

        const quill_payment_policy = new Quill('#editor_payment_policy', {
            theme: 'snow'
        });
        var quillEditor_payment_policy = document.getElementById('payment_policy');
        quillEditor_payment_policy.value = quill_payment_policy.root.innerHTML;
        quill_payment_policy.on('text-change', function() {
            quillEditor_payment_policy.value = quill_payment_policy.root.innerHTML;
        });
    </script>
@endsection
